...
let scenarios =
  [
    {
      description =
        "You receive a call about a driver weaving erratically through \
         traffic. You need to act quickly.";
      options =
        [
    ...
    {
      description =
        "A car is parked illegally in a handicapped spot without a permit. \
         While investigating, you find a clue that requires solving a \
         scrambled word puzzle to verify parking records.";
      options =
        [
      ...
      try
        ...
        if choice < 1 || choice > List.length scenario.options then (
          ...
          handle_scenario scenario points
            used_words (* Retry current scenario *))
        else
...
let play_traffic_cop () =
  let rec play_scenarios scenarios points =
    ...
    | scenario :: remaining_scenarios ->
        let updated_points, _ = handle_scenario scenario points [] in
