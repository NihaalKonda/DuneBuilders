...
let shuffle_word word =
  ...
  let shuffled_chars = 
    List.map snd (List.sort compare (List.map (fun c -> (Random.bits (), c)) chars))
  in
  String.concat "" (List.map (String.make 1) shuffled_chars)

(* Word bank for the game: police/law enforcement-related terms *)
let words = [
  "arrest"; "justice"; "investigation"; "patrol"; "detective"; 
  "evidence"; "warrant"; "forensics"; "suspect"; "crime"
]

(* Get a scrambled word and its correct answer, avoiding repeats *)
let rec get_scrambled_word used_words =
  let remaining_words = List.filter (fun word -> not (List.mem word used_words)) words in
  if remaining_words = [] then
    failwith "No more words available"
  else
    let word = List.nth remaining_words (Random.int (List.length remaining_words)) in
