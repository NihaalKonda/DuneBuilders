...
let rec ask_question question answer =
  ...
  try
    ...
    if user_answer = answer then true
      ...
      Printf.printf
        "Incorrect. The correct answer was %d. Try a new question!\n\n" answer;
      false
    end
  with Failure _ ->
...
let run_quiz () =
  ...
  let rec play streak =
    if streak = 5 then
      Printf.printf
        "Congratulations! You answered 5 questions correctly in a row and won \
         the game!\n"
    else begin
      let question, answer = generate_question () in
      if ask_question question answer then play (streak + 1)
        (* Increment streak if correct *)
      else play 0 (* Reset streak if incorrect *)
    end
  in
  play 0
